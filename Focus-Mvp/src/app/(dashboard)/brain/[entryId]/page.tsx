...
export default function EntryDetailPage() {
  ...
  const versions = useMemo(
    () =>
      MOCK_VERSION_HISTORY.filter((v) => v.entryId === entryId).sort(
    ...
    <div className="space-y-6 w-full max-w-4xl mx-auto">
      ...
      {isExpired && (
        <div className="flex items-center justify-between bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
          <div className="flex items-center gap-2 text-sm text-amber-800">
            ...
            This entry expired on {formatShortDate(entry.expiryDate!)} and is no
            longer included in AI answers.
          </div>
      ...
      <div>
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem
                onClick={() => router.push(`/brain/${entry.id}/edit`)}
              >
        ...
        <div className="flex items-center gap-2 mt-3 text-sm text-slate-400 flex-wrap">
          <div className="w-6 h-6 rounded-full bg-gradient-to-br from-slate-400 to-slate-500 flex items-center justify-center shrink-0">
        ...
        <div className="flex items-center gap-2 mt-1 text-sm text-slate-400">
          <WeightDotsInline weight={entry.aiContextWeight} />
          ...
          <span>
            {entry.expiryDate
              ? `Expires ${formatShortDate(entry.expiryDate)}`
              : 'No expiry'}
          </span>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {entry.conditions.length > 0 && (
            <StructuredPanel
              ...
              bgClass={config.tailwindBg}
              borderClass={config.tailwindBorder}
            />
          )}
          {entry.actions.length > 0 && (
            <StructuredPanel
      ...
      {entry.tags.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {entry.tags.map((tag) => (
            <span
              key={tag}
              className="bg-slate-100 text-slate-600 text-xs rounded-full px-2.5 py-1"
            >
              {tag}
            </span>
          ))}
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6">
        ...
        <p className="text-xs text-slate-500 mb-4">
          How this entry appears to Focus AI in every conversation
        </p>
        ...
        <p className="text-xs text-slate-400 italic mt-3">
          When you ask Focus AI a question, answers are shaped by entries like
          this one — cited by name.
        </p>
      </div>
                      ...
                      >
                        ...
                        <ChevronDown
                          className={cn(
                            ...
                            expandedVersions[v.id] && 'rotate-180'
                          )}
                        />
                      </button>
                    )}
                    {expandedVersions[v.id] && v.version > 1 && (
                      <DiffPanel version={v.version} />
                    )}
                  </div>
                </div>
              ))}
...
function WeightDotsInline({ weight }: { weight: AIContextWeight }) {
...
function DiffPanel({ version }: { version: number }) {
  const diffs: Record<number, { before: string; after: string }> = {
    3: {
      before: '...no single supplier may exceed 35%...',
      after: '...no single supplier may exceed 40%...',
    },
    2: {
      before: '(No exception clause for thin supplier markets)',
      after:
        'Categories with fewer than 3 qualified suppliers (documented exception required)',
    },
  }
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-6 animate-in slide-in-from-bottom-2 duration-200">
      <h3 className="text-sm font-semibold text-slate-900 mb-4">
        Propose a Change to This Entry
      </h3>
      <div className="space-y-4">
        <div className="space-y-1.5">
          ...
          <Input
            ...
            placeholder="Short title for this change proposal"
        ...
        <div className="space-y-1.5">
          ...
          <Textarea
            ...
            placeholder="Paste the updated body content here"
            rows={5}
            className="rounded-lg"
          />
        </div>
      ...
      <div className="flex items-center justify-between mt-4">
        ...
        <Button
          ...
          disabled={!title.trim() || !reason.trim() || !proposedBody.trim()}
        >
          Submit Proposal
        </Button>
      </div>
